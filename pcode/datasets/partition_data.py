...
class DataSampler(object):
    ...
    def get_sampled_indices(self, random_state = np.random.RandomState(7)):
        indices = self.indices
        if self.conf.graph.rank == 0:
            indices = self.sample_indices(random_state)

        if self.conf.aux_data:
            self.sampled_indices = self._get_consistent_indices(indices, random_state)

    def sample_indices(self, random_state = None):
        if random_state is None:
            random_state = self.conf.random_state

        if self.data_scheme == "random_sampling":
            self.sampled_indices = random_state.choice(
                self.indices,
    ...
    def _get_consistent_indices(self, indices,random_state = None):
        if random_state is None:
...
class DataPartitioner(object):
    ...
    def partition_indices(self, indices):
        ...
        self.targets_of_partitions = record_class_distribution(
            self.partitions, self.data.targets, print_fn=self.conf.logger.log,rank=self.conf.graph.rank
        )
        # if len(self.partitions) == 20:
        #     for idx in range(20):
        #         client_data = self.use(idx)
        ...
        #             torch.save(input,str(idx)+'_input.pt')
        #             torch.save(target,str(idx)+'_target.pt')

        # except:
        #     print("failed to display the distribution")
...
def record_class_distribution(partitions, targets, print_fn, rank):
